...
namespace UI
{
    public class PauseMenu : MonoBehaviour, IInputListener
    {
        ...
        {
            if (GameManager.Instance != null) GameManager.Instance.GetManager<InputManager>().RegisterListener(this);
        }

        private void OnDisable()
        {
            if (GameManager.Instance != null) GameManager.Instance.GetManager<InputManager>().UnregisterListener(this);
        }

        public void OnInputEvent(InputEvents inputEvent, InputState state)
        {
            if (inputEvent == InputEvents.Pause && state == InputState.Started) TogglePause();
        }
