...
namespace ShaderHelper.Shadow2D
{
    public class ShadowRTDrawer : MonoBehaviour
    {
        ...
        [Header("View Settings")] public float viewWidth = 10f;

        public float viewHeight = 10f;
        public float nearPlane = -10f;
        public float farPlane = 10f;

        [Header("Render Settings")] public Material whiteMat;

        public List<ShadowObject> casters = new();
        public RenderTexture shadowRT;

        [HideInInspector] public UnityEngine.Camera shadowCamera;

        private readonly Dictionary<Sprite, Mesh> meshCache = new();
        ...
        public Mesh GetMesh(Sprite sprite)
        {
            ...
            if (meshCache.TryGetValue(sprite, out var m))
                return m;
