...
namespace Customer
{
    ...
    {
        [Header("Movement Parameters")] [SerializeField] [Tooltip("Maximum horizontal speed (m/s)")]
        ...
        [Header("Animation Control (Optional)")] [SerializeField] [Tooltip("Character animator component")]
        ...
        private void FixedUpdate()
        {
            ...
            var distanceToTarget = TargetX - transform.position.x;
            var direction = Mathf.Sign(distanceToTarget);
            var currentSpeed = _rb.velocity.x;
            ...
            var stoppingDistance = currentSpeed * currentSpeed / (2f * deceleration);
            ...
            if (Mathf.Abs(distanceToTarget) > stoppingDistance)
            {
                ...
                var newSpeed =
                    Mathf.MoveTowards(currentSpeed, direction * maxSpeed, acceleration * Time.fixedDeltaTime);
                _rb.velocity = new Vector2(newSpeed, _rb.velocity.y);
            }
            else
            {
                // Deceleration phase - need to slow down to stop at target
                var newSpeed = Mathf.MoveTowards(currentSpeed, 0f, deceleration * Time.fixedDeltaTime);
                _rb.velocity = new Vector2(newSpeed, _rb.velocity.y);
            }
        ...
        private void UpdateAnimation()
        {
            ...
            if (animator != null) animator.SetBool("RUN", _isMoving);
            ...
            if (spriteRenderer != null && _isMoving)
            {
                ...
                else if (velocityX < -0.01f) spriteRenderer.flipX = false; // Moving left
            }
        }
    }
}