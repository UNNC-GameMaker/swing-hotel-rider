...
namespace Building
{
    ...
    {
        [Header("Building Configuration")] public Vector2Int position;

        [SerializeField] [Tooltip("The size of the building in grid cells")]
        private Vector2Int roomSize;

        [Header("Tile References")] [SerializeField] [Tooltip("Tile used for room interior")]
        ...
        [Header("References")] [SerializeField] [Tooltip("The tilemap to place building tiles on")]
        ...
        private void Awake()
        {
            ...
            if (tilemap == null)
            {
                var colliderTileObj = GameObject.Find("ColliderTile");
                if (colliderTileObj != null) tilemap = colliderTileObj.GetComponent<Tilemap>();
            }
        ...
        {
            foreach (var rb in _activatedRigidbodies.ToArray())
            {
            ...
            foreach (var rb in _rigidbodies.ToArray())
            {
                ...
                var baseFurniture = Instantiate(rb.gameObject, transform);
                var rbNew = baseFurniture.GetComponent<Rigidbody2D>();
        ...
        private void PlaceTiles()
        {
            ...
            var tilePosition = position * _gridManager.GridSize;
        ...
        private void PlaceWalls(Vector2Int tilePosition)
        {
            ...
            for (var j = -1; j <= gridSize.y; j++)
            {
                var pos = new Vector3Int(tilePosition.x - 1, tilePosition.y + j, 0);
                if (tilemap.GetTile(pos) == null) tilemap.SetTile(pos, solidTile);
            }

            // Right wall (x = size.x, all y from -1 to size.y)
            for (var j = -1; j <= gridSize.y; j++)
            {
                var pos = new Vector3Int(tilePosition.x + gridSize.x, tilePosition.y + j, 0);
                if (tilemap.GetTile(pos) == null) tilemap.SetTile(pos, solidTile);
            }
            ...
            {
                var pos = new Vector3Int(tilePosition.x + i, tilePosition.y - 1, 0);
                if (tilemap.GetTile(pos) == null) tilemap.SetTile(pos, solidTile);
            }
            ...
            for (var i = -1; i <= gridSize.x; i++)
            {
                var pos = new Vector3Int(tilePosition.x + i, tilePosition.y + gridSize.y, 0);
                if (tilemap.GetTile(pos) == null) tilemap.SetTile(pos, solidTile);
            }
        }
        ...
        private void PlaceFloor(Vector2Int tilePosition)
        {
            ...
            {
                var pos = new Vector3Int(tilePosition.x + i, tilePosition.y + j, 0);
        ...
        private void PlaceBottomEntrance(Vector2Int tilePosition)
        {
            ...
            for (var i = 0; i < gridSize.x; i++)
            {
                ...
                if (tilemap.GetTile(pos) == roomTile) tilemap.SetTile(pos, oneWayPlatformTile);
            }
        }
