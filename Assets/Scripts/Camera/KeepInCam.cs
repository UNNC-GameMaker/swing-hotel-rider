...
namespace Camera
{
    ...
    public class KeepInCam : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("Target orthographic camera. Defaults to Camera.main if not specified.")]
        private UnityEngine.Camera targetCamera;

        [Header("Viewport Padding")]
        [SerializeField]
        [Tooltip("Margin in viewport coordinates (0~0.5) to leave space around edges.")]
        private Vector2 viewportPadding = new(0.02f, 0.02f);

        [Header("Behavior")] [Tooltip("Enable/disable keeping object in camera view")]
        ...
        private void LateUpdate()
        {
            ...
            if (_parentTransform == null || targetCamera == null) return;
            ...
            var desiredWorldPosition = _parentTransform.position;

            // 2) Convert to Viewport coordinates: bottom-left (0,0), top-right (1,1)
            var viewportPosition = targetCamera.WorldToViewportPoint(desiredWorldPosition);

            // 3) Check if already visible within bounds (considering padding)
            var isWithinXBounds = viewportPosition.x >= viewportPadding.x &&
                                  viewportPosition.x <= 1f - viewportPadding.x;
            var isWithinYBounds = viewportPosition.y >= viewportPadding.y &&
                                  viewportPosition.y <= 1f - viewportPadding.y;
            var isWithinDepthBounds = viewportPosition.z >= targetCamera.nearClipPlane &&
            ...
            var clampedWorldPosition = targetCamera.ViewportToWorldPoint(viewportPosition);
